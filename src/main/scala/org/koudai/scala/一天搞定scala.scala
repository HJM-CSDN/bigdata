...
/**
  ...
  *     println()
  *     for(x <- myList2){
  *       print(" " + x)  // 输出：10  12 13 14 15 16 17 18 19
  *     }
  *   }
  * }
  */
...
object Test3 {
  def main(args: Array[String]) {
    val writer = new PrintWriter(new File("D:\\test.txt" ),"utf-8")
    for(i <- 1 to 10)
    writer.write(i)
    writer.write("你好啊")
    writer.close()
...
object Test {
  def main(args: Array[String]) {
    println("文件内容为:" )
    //文件中如果出现中文会有问题
    Source.fromFile("C:\\Users\\hjm13\\Desktop\\数据清洗\\项目\\1.txt" ,"gbk").foreach{
      print
    }
  }
}